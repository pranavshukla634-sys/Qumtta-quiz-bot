...
async def pause_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.id != GROUP_ID or update.effective_user.id != OWNER_ID:
        return
    if is_stopped:
...
async def admin_time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == 'single':
        job = context.job_queue.run_once(
            ...
            when=int(delay),
            data={'quiz_id': quiz_id, 'mode': 'single', 'initiator': update.effective_user.id}
        )
        ...
        job = context.job_queue.run_once(
            start_scheduled_quiz,
            when=int(delay),
            data={'quiz_id': quiz_id, 'mode': 'all', 'initiator': update.effective_user.id}
        )
        # === STORE IN SCHEDULED LIST ===
        scheduled_quizzes.append({
            'quiz_id': quiz_id,
...
async def start_scheduled_quiz(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(OWNER_ID, f"✅ Scheduled quiz triggered at {datetime.now().strftime('%H:%M:%S')}")
